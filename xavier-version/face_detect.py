...
def camThread(frameBuffer, results, MJPEGQueue, stop_prog, file2log):
    capture_width=3264
    capture_height=2464
    frame_rate=30
    flip_method=2
    ...
    gstreamer_pipeline = ('v4l2src device=/dev/video0 do-timestamp=true !  video/x-raw, width=1280, height=720, framerate=30/1 ! tee name=streams streams. ! nvvidconv ! videoconvert ! video/x-raw, format=BGR ! appsink')
...
def recognition(frameBuffer, objsBuffer, namesBuffer, stop_prog):
...
if __name__ == '__main__':
    ...
    camProc = Process(target=camThread, args=(recImage, resultRecogn, MJPEGQueue,  prog_stop, 'test.avi'), daemon=True)
    camProc.start()
    frecogn = Process(target=recognition, args=(recImage, resultRecogn, NamesQueue, prog_stop), daemon=True)
    frecogn.start()
